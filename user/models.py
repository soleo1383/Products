...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    phone_number = models.BigIntegerField('phone number', unique=True, null=True, blank=True,
                                          validators=[
    ...
    def email_user(self, subject, massage, from_email=None, **kwargs):
        send_mail(subject, massage, from_email, [self.email], **kwargs)

    @property
    def is_loggined_user(self):
...
class UserProfile(models.Model):
    ...
    nick_name = models.CharField('nick name', max_length=150, blank=True)
    avatar = models.ImageField('avatar', blank=True)
    birthday = models.DateField('birthday', null=True, blank=True)
    gender = models.BooleanField('gender', null=True, blank=True)
    province = models.ForeignKey(verbose_name='province', to='Province', null=True, on_delete=models.SET)
    #email = models.EmailField('email number', null=True, blank=True)
...
class Device(models.Model):
    ...
    notify_token = models.CharField('Notification Token', max_length=200, blank=True,
                                    validators=[
                                        validators.RegexValidator(r'([a-z]|[A-z]|[0-9])\w+',
                                                                  'Notify token is not valid'
                                                                  )
                                    ],
                                    )
